...
export const generateScene = async (medicalText: string): Promise<GenerateResult> => {
  ...
  const fullPrompt = `${scene_prompt} Style: sketchy, mnemonic, visual metaphor. ULTRA-CRITICAL RULE: The image must contain ZERO text, ZERO letters, ZERO numbers. It must be a purely visual scene. Do NOT write on the image. This is a strict, non-negotiable instruction.`;
